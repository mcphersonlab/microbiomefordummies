...
The mini-lab below is **interactive and intentionally synthetic**: it is there to help you build intuition before you run the R code farther down the page on your own tables.
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function renderAlpha() {
    ...
    const segments = composition.map((value, index) => {
      ...
      const rect = `<rect x="${offset}" y="20" width="${width}" height="36" rx="5" ry="5" fill="${palette[index % palette.length]}"></rect>`;
      const label = width > 7
        ? `<text x="${offset + width / 2}" y="43" text-anchor="middle" font-size="11" fill="white" font-weight="700">T${index + 1}</text>`
        : "";
      offset += width;
      return rect + label;
    }).join("");
  ...
  function jaccard(a, b) {
    ...
    let union = 0;
    for (let i = 0; i < a.length; i += 1) {
      const presentA = a[i] > 0.08 ? 1 : 0;
      const presentB = b[i] > 0.08 ? 1 : 0;
