...
document.addEventListener("DOMContentLoaded", function () {
  ...
  diversityLab.innerHTML = `
    <section class="eco-baseline-bridge" aria-labelledby="eco-baseline-heading">
      <h3 id="eco-baseline-heading">Shared baseline data characteristics</h3>
      <p class="eco-caption">The same reference community anchors both examples below: alpha diversity compresses one baseline-like sample into richness and evenness metrics, while beta diversity shows how multiple samples move away from that starting composition in high-dimensional space.</p>
    ...
    <div class="eco-diversity-grid">
      <section class="eco-card" aria-labelledby="alpha-diversity-heading">
        ...
        <p class="eco-caption">Change the number of observed taxa and how dominant the largest taxon is. The chart keeps the shared baseline visible so you can see how one sample’s composition translates into alpha-diversity summaries.</p>
      ...
      <section class="eco-card" aria-labelledby="beta-diversity-heading">
        ...
        <p class="eco-caption">Move the treatment group farther away from the controls, then switch both the <strong>Distance Metric</strong> and the <strong>Ordination and visualization</strong> algorithm to see how the same high-dimensional baseline pattern is summarized.</p>
        ...
        <div class="eco-inline-legend" aria-hidden="true">
          <span class="eco-legend-item"><span class="eco-legend-swatch eco-legend-swatch--baseline"></span>Baseline reference</span>
          <span class="eco-legend-item"><span class="eco-legend-swatch eco-legend-swatch--control"></span>Control samples</span>
          <span class="eco-legend-item"><span class="eco-legend-swatch eco-legend-swatch--treatment"></span>Treatment samples</span>
        </div>
  ...
  const SIX_TAXON_BASELINE_PROFILE = normalize([0.34, 0.22, 0.18, 0.14, 0.08, 0.04]);
  ...
  function renderSharedBaseline() {
    baselineChart.innerHTML = `
      <svg viewBox="0 0 320 122" width="100%" role="img" aria-label="Shared baseline community that connects the alpha and beta diversity examples">
        <text x="30" y="16" font-size="10" fill="#334155" font-weight="700">Baseline reference community (T1-T6)</text>
        ${stackedBarSegments(SIX_TAXON_BASELINE_PROFILE, { x: 30, y: 24, width: 260, height: 20 })}
        <text x="30" y="58" font-size="8.5" fill="#475569">These same baseline data characteristics feed both graphics below.</text>
        ...
        <text x="238" y="99" text-anchor="middle" font-size="7.5" fill="#1f2937">Many samples → distance + ordination</text>
      </svg>
    `;
  }
  ...
  function renderAlpha() {
    ...
    alphaCompositionChart.innerHTML = `
      <svg viewBox="0 0 100 104" width="100%" role="img" aria-label="Baseline and current single-sample composition for the alpha-diversity example">
        <text x="4" y="12" font-size="7.5" fill="#334155">Baseline reference profile</text>
        ${stackedBarSegments(SIX_TAXON_BASELINE_PROFILE, { x: 4, y: 18, width: 92, height: 18 })}
        <line x1="50" y1="37" x2="50" y2="48" stroke="#94a3b8" stroke-width="0.7" stroke-dasharray="2 2"></line>
        <text x="4" y="57" font-size="7.5" fill="#334155">Current single-sample composition</text>
        ${stackedBarSegments(composition, { x: 4, y: 63, width: 92, height: 18 })}
        <text x="4" y="92" font-size="7" fill="#475569">Each segment is one detected taxon; higher dominance lowers evenness even when richness stays constant.</text>
      </svg>
    `;
  }
  ...
  function buildBetaSamples(shiftStrength) {
    const treatmentSignal = [-0.16, -0.10, -0.03, 0.08, 0.11, 0.10];

    return [
      { name: "Baseline", group: "Baseline", values: SIX_TAXON_BASELINE_PROFILE },
      { name: "Control 1", group: "Control", values: shiftedProfile(SIX_TAXON_BASELINE_PROFILE, [0.01, -0.01, 0.00, 0.00, 0.01, -0.01], 0.35, [0.005, -0.002, -0.001, 0.002, -0.001, -0.003]) },
      { name: "Control 2", group: "Control", values: shiftedProfile(SIX_TAXON_BASELINE_PROFILE, [-0.01, 0.01, 0.00, 0.01, -0.01, 0.00], 0.35, [-0.003, 0.004, -0.002, 0.002, 0.001, -0.002]) },
      { name: "Control 3", group: "Control", values: shiftedProfile(SIX_TAXON_BASELINE_PROFILE, [0.00, -0.01, 0.01, -0.01, 0.01, 0.00], 0.35, [0.001, -0.002, 0.003, -0.002, 0.001, -0.001]) },
      { name: "Treatment 1", group: "Treatment", values: shiftedProfile(SIX_TAXON_BASELINE_PROFILE, treatmentSignal, shiftStrength, [0.002, -0.003, 0.001, 0.002, -0.001, -0.001]) },
      { name: "Treatment 2", group: "Treatment", values: shiftedProfile(SIX_TAXON_BASELINE_PROFILE, treatmentSignal, shiftStrength * 1.08, [-0.002, 0.002, -0.001, 0.001, 0.003, -0.001]) },
      { name: "Treatment 3", group: "Treatment", values: shiftedProfile(SIX_TAXON_BASELINE_PROFILE, treatmentSignal, shiftStrength * 0.92, [0.001, -0.001, 0.000, -0.002, 0.003, -0.001]) }
    ];
  }
  ...
  function distanceMatrix(samples, metric) {
    return samples.map((rowSample, rowIndex) =>
      samples.map((colSample, colIndex) =>
        rowIndex === colIndex ? 0 : betaDistance(rowSample.values, colSample.values, metric)
      )
    );
  }
  ...
  function renderBeta() {
    ...
    const distances = distanceMatrix(samples, metric);
    ...
    const treatmentPoints = projectedPoints.filter((_, index) => samples[index].group === "Treatment");
    ...
    const treatmentCentroid = {
      x: treatmentPoints.reduce((sum, point) => sum + point.x, 0) / treatmentPoints.length,
      y: treatmentPoints.reduce((sum, point) => sum + point.y, 0) / treatmentPoints.length
    };
    ...
    const centroidAnnotations = `
      ...
      <line x1="${baselinePoint.x}" y1="${baselinePoint.y}" x2="${treatmentCentroid.x}" y2="${treatmentCentroid.y}" stroke="#c62828" stroke-width="1.8"></line>
      <text x="${controlCentroid.x + 8}" y="${controlCentroid.y - 8}" font-size="8" fill="#0f172a">Control centroid</text>
      <text x="${treatmentCentroid.x + 8}" y="${treatmentCentroid.y - 8}" font-size="8" fill="#0f172a">Treatment centroid</text>
    `;
    ...
    betaOrdinationChart.innerHTML = `
      <svg viewBox="0 0 340 235" width="100%" role="img" aria-label="High-dimensional beta diversity ordination with baseline, control, and treatment samples">
        <text x="12" y="16" font-size="10" fill="#334155" font-weight="700">${ordinationLabels[ordination]} view of the same high-dimensional community shift</text>
        <text x="12" y="30" font-size="8" fill="#475569">Distance metric: ${metricLabels[metric]}. Dashed trajectories connect the shared baseline data characteristics to each derived sample.</text>
        ${axisX}
        ${axisY}
    ...
    const nonBaselineDistances = distanceMatrix(nonBaselineSamples, metric);
    const within = [];
    const between = [];
    ...
    betaSummary.textContent = `${metricLabels[metric]} + ${ordinationLabels[ordination]}: mean within-group distance = ${formatNumber(meanWithin)} and mean between-group distance = ${formatNumber(meanBetween)}. ${ordinationDescriptions[ordination]}`;
  }
  ...
  betaOrdinationInput.addEventListener("change", renderBeta);
