...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const baselineProfile = normalize([0.34, 0.22, 0.18, 0.14, 0.08, 0.04]);
  ...
  function renderSharedBaseline() {
    baselineChart.innerHTML = `
      <svg viewBox="0 0 320 122" width="100%" role="img" aria-label="Shared baseline community that connects the alpha- and beta-diversity examples">
        ...
        ${stackedBarSegments(baselineProfile, { x: 30, y: 24, width: 260, height: 20 })}
  ...
  function renderAlpha() {
    ...
    alphaCompositionChart.innerHTML = `
      <svg viewBox="0 0 100 104" width="100%" role="img" aria-label="Baseline and current single-sample composition for the alpha-diversity example">
        ...
        ${stackedBarSegments(baselineProfile, { x: 4, y: 18, width: 92, height: 18 })}
  ...
  function buildBetaSamples(shiftStrength) {
    ...
    return [
      { name: "Baseline", group: "Baseline", values: baselineProfile },
      { name: "Control 1", group: "Control", values: shiftedProfile(baselineProfile, [0.01, -0.01, 0.00, 0.00, 0.01, -0.01], 0.35, [0.005, -0.002, -0.001, 0.002, -0.001, -0.003]) },
      { name: "Control 2", group: "Control", values: shiftedProfile(baselineProfile, [-0.01, 0.01, 0.00, 0.01, -0.01, 0.00], 0.35, [-0.003, 0.004, -0.002, 0.002, 0.001, -0.002]) },
      { name: "Control 3", group: "Control", values: shiftedProfile(baselineProfile, [0.00, -0.01, 0.01, -0.01, 0.01, 0.00], 0.35, [0.001, -0.002, 0.003, -0.002, 0.001, -0.001]) },
      { name: "Treatment 1", group: "Treatment", values: shiftedProfile(baselineProfile, treatmentSignal, shiftStrength, [0.002, -0.003, 0.001, 0.002, -0.001, -0.001]) },
      { name: "Treatment 2", group: "Treatment", values: shiftedProfile(baselineProfile, treatmentSignal, shiftStrength * 1.08, [-0.002, 0.002, -0.001, 0.001, 0.003, -0.001]) },
      { name: "Treatment 3", group: "Treatment", values: shiftedProfile(baselineProfile, treatmentSignal, shiftStrength * 0.92, [0.001, -0.001, 0.000, -0.002, 0.003, -0.001]) }
    ];
  }
  ...
  function normalizedCoords(coords) {
    ...
    return centered.map((point, index) => [
      point[0] / maxAbs + 0.02 * Math.cos(index),
      point[1] / maxAbs + 0.02 * Math.sin(index)
    ]);
  }
  ...
  function rankDisparities(distances) {
    ...
    pairs.forEach((pair, index) => {
      pair.target = 0.25 + (index / denominator) * 1.15;
    });
    return pairs;
  }
