...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function buildTreatmentSignal(length) {
  ...
  function interventionPreviewProfile(baseProfile) {
    ...
    return shiftedProfile(
      baseProfile,
      buildTreatmentSignal(baseProfile.length),
      shiftStrength,
  ...
  function buildBetaSamples(baseProfile, shiftStrength, variability, sampleCount) {
    const treatmentSignal = buildTreatmentSignal(baseProfile.length);
    const controlSignal = treatmentSignal.map((value) => value * 0.14);
  ...
  betaMetricInput.addEventListener("change", renderAll);
  betaOrdinationInput.addEventListener("change", renderAll);
