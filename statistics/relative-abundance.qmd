...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function renderLevel(levelName, contrastName) {
    ...
    legend.innerHTML = level.taxa.map((taxon) => {
      const significant = taxon.comparisons[contrastName].q < 0.05 ? " • significant in selected contrast" : "";
      return `
    ...
    const significanceRows = level.taxa.map((taxon, index) => {
      ...
      const qLabel = comparison.q < 0.05 ? `q=${comparison.q.toFixed(3)}` : "ns";
      const point = comparison.q < 0.05
      ...
      return `
        ...
        <rect x="${x}" y="${y - 8}" width="${effectWidth}" height="16" fill="${taxon.color}" opacity="${comparison.q < 0.05 ? 0.95 : 0.45}" rx="4" ry="4"></rect>
        ${point}
    ...
    const significantTaxa = level.taxa
      .filter((taxon) => taxon.comparisons[contrastName].q < 0.05 && taxon.name !== "Other");
    const significantReplicateChecks = level.replicateChecks.filter((check) => check.q < 0.05);
