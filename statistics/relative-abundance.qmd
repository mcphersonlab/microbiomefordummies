...
The interactive explorer below now uses a **longitudinal control-versus-experimental design** with triplicate subjects at **time -3**, **Pre- (time 0)**, and **time +4**. You can change taxonomic level, inspect the sample-code key, and watch both the stacked bars and the integrated comparison framework update together.

My recommended visualization idea is the **taxa-preserving comparison framework**:

- the stacked bars keep taxa-specific color as the primary encoding
- a second aligned panel keeps the four core longitudinal comparisons visible at the same time
- significant cells keep their `q` labels, but taxa **keep the same taxon color** instead of switching to a significance palette
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  abundanceLab.innerHTML = `
    ...
    <div class="ra-lab-grid">
      ...
      <section class="ra-card">
        <h3>Integrated statistical comparison framework</h3>
        <p class="ra-caption">This companion panel synthesizes Pre- group matching, control stability, experimental response, and time +4 group separation so every taxon is interpreted against the full longitudinal design without losing the taxon colors used in the stacked bars.</p>
        <div id="ra-framework-overview" class="ra-framework-overview" aria-live="polite"></div>
  ...
  const frameworkOverview = document.getElementById("ra-framework-overview");
  ...
  const comparisonFramework = [
    {
      key: "baseline_group",
      label: "1. Pre- group match",
      emphasis: "Expect non-significant",
      detail: "Control-group and experimental-group should still match at time 0."
    },
    {
      key: "control_time",
      label: "2. Control stability",
      emphasis: "Expect non-significant",
      detail: "Control-group drift from Pre- to time +4 should stay small."
    },
    {
      key: "intervention_time",
      label: "3. Experimental response",
      emphasis: "Look for significance",
      detail: "Experimental-group Pre- to time +4 captures the within-arm intervention signal."
    },
    {
      key: "post_group",
      label: "4. Time +4 separation",
      emphasis: "Look for significance",
      detail: "Control-group and experimental-group should separate by time +4."
    }
  ];

  const cmp = (effect, q, enriched) => ({ effect, q, enriched });

  const average = (left, right) => Number(((left + right) / 2).toFixed(1));

  function expandTriplicateScores(scores) {
    const [controlPreA, controlPreB, controlPostA, controlPostB, experimentalPreA, experimentalPreB, experimentalPostA, experimentalPostB] = scores;
    return [
      controlPreA, average(controlPreA, controlPreB), controlPreB,
      average(controlPreA, controlPreB), controlPreA, controlPreB,
      controlPostA, average(controlPostA, controlPostB), controlPostB,
      experimentalPreA, average(experimentalPreA, experimentalPreB), experimentalPreB,
      average(experimentalPreA, experimentalPreB), experimentalPreA, experimentalPreB,
      experimentalPostA, average(experimentalPostA, experimentalPostB), experimentalPostB
    ];
  }
  ...
  Object.values(levels).forEach((level) => {
    level.taxa.forEach((taxon) => {
      taxon.scores = expandTriplicateScores(taxon.scores);
    });
  });
  ...
  function normalizeComparisonLabel(label) {
    return label
      .replaceAll("Intervention", "Experimental")
      .replaceAll("Control baseline", "Control Pre-")
      .replaceAll("Experimental baseline", "Experimental Pre-")
      .replaceAll("Control post", "Control +4")
      .replaceAll("Experimental post", "Experimental +4");
  }
  ...
  function renderLevel(levelName) {
    ...
    legend.innerHTML = level.taxa.map((taxon) => {
      ...
      const significant = frameworkCall.label === "Intervention-linked" ? " • passes integrated framework" : "";
      return `
    ...
    frameworkOverview.innerHTML = comparisonFramework.map((item) => `
      <div class="ra-framework-step">
        <strong>${item.label}</strong>
        <span><em>${item.emphasis}</em></span>
