...
document.addEventListener("DOMContentLoaded", function () {
  ...
  function computeLog2FoldChange(preValues, postValues) {
    return Math.log2(
      (mean(postValues) + VOLCANO_LOG2FC_PSEUDOCOUNT) /
      (mean(preValues) + VOLCANO_LOG2FC_PSEUDOCOUNT)
    );
  }
  ...
  function computeSignedLdaLogScore(preValues, postValues) {
    ...
    // Convert standardized separation into a bounded signed score on a 0-4 scale for any
    // later LEfSe-style overlay work without overwhelming point sizes.
  ...
  function renderLevel(levelName, contrastName) {
    ...
    const volcanoXLimit = Math.max(1, Math.ceil(Math.max(...volcanoPoints.map((point) => Math.abs(point.log2FoldChange))) * 2) / 2);
    const volcanoTickStep = volcanoXLimit <= 1.5 ? 0.5 : volcanoXLimit <= 3 ? 1 : 2;
    ...
    const volcanoXTicks = Array.from(
      { length: Math.round((volcanoXLimit * 2) / volcanoTickStep) + 1 },
      (_, index) => Number((-volcanoXLimit + (index * volcanoTickStep)).toFixed(2))
    ).map((tick) => {
    ...
    const volcanoPointMarks = volcanoPoints.map((point) => {
      ...
      return `
        <g>
          <title>${escapeSvgText(point.taxon.name)}: log2 fold change ${point.log2FoldChange.toFixed(2)}, p=${point.pValue.toFixed(4)}, q=${point.comparison.q.toFixed(3)}</title>
          <circle
            cx="${x}"
            cy="${y}"
...
const circles = volcanoPoints.map((point) => `
  <circle
    ...
    r="4.8"
    fill="${point.taxon.color}"
    stroke="${point.y >= -Math.log10(0.05) ? "#0f172a" : "#94a3b8"}"
  ></circle>
`);
```
