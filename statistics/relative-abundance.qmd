...
document.addEventListener("DOMContentLoaded", function () {
  ...
  // The page only stores synthetic q-values for each contrast, so derive a bounded display-only
  // p-value from the same ranking to support the requested volcano-plot geometry.
  function deriveSyntheticPValue(q) {
    return Math.max(Math.min(q * 0.5, 0.99), 0.0005);
  }
  ...
  function renderLevel(levelName, contrastName) {
    ...
    const volcanoPoints = level.taxa.map((taxon) => {
      ...
      const normalizedEffect = Math.max(-normalizedEffectLimit, Math.min(normalizedEffectLimit, (comparison.effect / maxEffect) * normalizedEffectLimit));
    ...
    const volcanoXTicks = [-0.5, -0.25, 0, 0.25, 0.5].map((tick) => {
      ...
      return `
        ...
        <text x="${x}" y="${volcanoBottom + 18}" text-anchor="middle" font-size="8" fill="#475569">${tick.toFixed(2).replace(/\.00$/, ".0").replace(/0$/, "")}</text>
      `;
    }).join("");
    const volcanoYTicks = [0, 1, 2, 3].filter((tick) => tick <= Math.ceil(maxNegativeLogP)).map((tick) => {
    ...
    const volcanoLabels = volcanoPoints
      .filter((point) => point.comparison.q < SIGNIFICANCE_THRESHOLD && point.taxon.name !== "Other")
      .map((point, index) => {
        const x = xForEffect(point.normalizedEffect);
        const y = yForLogP(point.negativeLogP) - (index % 2 === 0 ? 10 : -18);
        return `<text x="${x + (point.normalizedEffect >= 0 ? 8 : -8)}" y="${y}" text-anchor="${point.normalizedEffect >= 0 ? "start" : "end"}" font-size="8" fill="#334155">${point.taxon.name}</text>`;
      }).join("");
    const volcanoPointMarks = volcanoPoints.map((point) => {
      ...
      return `
        <g>
          <title>${point.taxon.name}: normalized effect ${point.normalizedEffect.toFixed(2)}, p=${point.pValue.toFixed(4)}, q=${point.comparison.q.toFixed(3)}</title>
