...
document.addEventListener("DOMContentLoaded", function () {
  ...
  abundanceLab.innerHTML = `
    ...
    <div class="ra-lab-grid">
      ...
      <section class="ra-card">
        ...
        <p class="ra-caption">This companion panel behaves like a LEfSe-style effect-size plot, but now you can switch between between-group and within-group longitudinal contrasts without losing the taxon colors used in the stacked bars.</p>
  ...
  function renderLevel(levelName, contrastName) {
    ...
    replicateChecks.innerHTML = level.replicateChecks.map((check) => `
      <div class="ra-replicate-chip">
        <strong>${check.label}</strong>
        <span>Same-group / same-time-point pairwise check: q=${check.q.toFixed(3)} · non-significant</span>
      </div>
    `).join("");

    if (significantTaxa.length === 0) {
      summary.textContent = `${contrast.description} At the ${levelName} level, no taxa reach multiple-testing significance in this selected contrast, and all same-group / same-time-point replicate checks also remain non-significant.`;
      return;
    }
    ...
    summary.textContent = `${contrast.description} At the ${levelName} level, the strongest significant shift is ${strongest.name} (${strongestComparison.enriched}-enriched, LEfSe-like effect ${strongestComparison.effect.toFixed(1)}, q=${strongestComparison.q.toFixed(3)}). Same-group / same-time-point replicate checks remain non-significant.`;
  }
